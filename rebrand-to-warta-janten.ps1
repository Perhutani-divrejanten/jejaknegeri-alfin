# Rebrand dari Indonesia Daily ke Pusat Informasi
...
Write-Log -Message "===== REBRAND Pusat Informasi DIMULAI ====="
...
foreach ($file in $htmlFiles) {
    try {
        ...
        $replacements = @(
            @{ Old = "Indonesia Daily"; New = "Pusat Informasi" },
            @{ Old = "IndonesiaDaily"; New = "PusatInformasi" },
            @{ Old = "indonesiadaily"; New = "PusatInformasi" },
            # Social media links
            @{ Old = "https://twitter.com/indonesiadaily"; New = "https://twitter.com/PusatInformasi" },
            @{ Old = "https://facebook.com/indonesiadaily"; New = "https://facebook.com/PusatInformasi" },
            @{ Old = "https://instagram.com/indonesiadaily"; New = "https://instagram.com/PusatInformasi" },
            @{ Old = "https://youtube.com/@indonesiadaily"; New = "https://youtube.com/@PusatInformasi" },
            @{ Old = "https://linkedin.com/company/indonesiadaily"; New = "https://linkedin.com/company/PusatInformasi" },
            # Email
            @{ Old = "indonesiadaily@gmail.com"; New = "PusatInformasi@gmail.com" },
            # Logo references
            @{ Old = 'alt="IndonesiaDaily"'; New = 'alt="PusatInformasi"' }
        )
...
foreach ($file in $cssFiles) {
    try {
        ...
        # Color replacements
        $colorReplacements = @(
            @{ Old = "#F59E0B"; New = "#F59E0B" },          # Primary: kuning -> hijau
            @{ Old = "#F59E0B"; New = "#F59E0B" },
            @{ Old = "rgb(255, 204, 0)"; New = "rgb(245, 158, 11)" },
            @{ Old = "#31404B"; New = "#5F1F7F" },          # Secondary: abu -> biru
            @{ Old = "#78350F"; New = "#78350F" },          # Dark: hitam -> darkgreen
            # Additional color mappings if needed
            @{ Old = "#78350F"; New = "#78350F" }
        )
...
foreach ($file in $packageJsonFiles) {
    try {
        ...
        $replacements = @(
            @{ Old = """name"": ""indonesiadaily"""; New = """name"": ""PusatInformasi""" },
            @{ Old = """name"": ""indonesiadaily-article-generator"""; New = """name"": ""PusatInformasi-article-generator""" }
        )
...
$newColors = @("#F59E0B", "#78350F", "#5F1F7F")
$colorFound = @{}
...
Write-Log -Message ""
Write-Log -Message "Rebrand Pusat Informasi selesai ✅"
